    ...
    <main className="max-w-6xl mx-auto px-6 py-12">
      <section className="pt-8 pb-14">
        <div className="inline-block bg-blue-50 text-blue-700 text-sm font-medium px-3 py-1 rounded-md mb-4">
          CSE Blockchain Architecture Course
        </div>
        <h1 className="text-5xl md:text-6xl font-bold text-gray-900 mb-6 leading-tight">
          Learn Blockchain Architecture
        </h1>
        <p className="text-xl text-gray-600 mb-8 leading-relaxed max-w-3xl">
          Unit-wise course presentations covering the history of electronic cash, blockchain foundations, Nakamoto consensus, blockchain networks, and financial-sector applications.
        </p>
        <div className="flex gap-4 flex-wrap">
          <Link href={`/courses/blockchain/${materials[0].id}`} className="inline-flex items-center gap-2 bg-blue-600 text-white font-semibold py-3 px-6 rounded-lg hover:bg-blue-700 transition-colors">
            <BookOpen size={18} />
            Start Unit 1
          </Link>
      ...
      <section aria-labelledby="course-materials" className="bg-gray-50 -mx-4 px-4 py-10 md:rounded-2xl">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {materials.map((material, index) => (
            <Link key={material.id} href={`/courses/blockchain/${material.id}`} className="group bg-white p-5 rounded-xl border-2 border-gray-200 hover:border-blue-500 transition-all">
