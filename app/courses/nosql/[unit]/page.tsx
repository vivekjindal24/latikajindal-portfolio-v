...
export default async function UnitPage({ params }: UnitPageProps) {
  ...
  return (
    <main className="max-w-6xl mx-auto px-6 py-10">
      <Link href="/courses/nosql" className="inline-flex items-center gap-2 text-sm font-semibold text-blue-700 hover:underline mb-8">
        <ArrowLeft size={16} />
        All NoSQL materials
      </Link>
      ...
      <nav aria-label="Course material navigation" className="flex justify-between gap-4 mt-8 pt-6 border-t border-gray-200">
        {previousMaterial ? (
          <Link href={`/courses/nosql/${previousMaterial.id}`} className="text-sm text-gray-600 hover:text-blue-700">
            Previous: {previousMaterial.unit}
          </Link>
        ) : <span />}
        {nextMaterial && (
          <Link href={`/courses/nosql/${nextMaterial.id}`} className="inline-flex items-center gap-2 text-sm font-semibold text-blue-700 hover:underline">
